...
def detail():
    ...
    with st.container(border=True):
        ...
        if edu_level == "Senior High":
            ...
            tert_level = st.selectbox("Tertiary Level *", ["Level 100","Level 200", "Level 300", "Level 400", 
                        "Level 500", "Level 600", "Level 700", "Final Year"], index=None, placeholder="Select", key="tert11")
    ...
    if submit:
        ...
        try:
            ...
            if not existing_data.empty:
                if contact.strip() in existing_data['Contact'].values:
                    errors.append("Contact number already registered")
                if email.strip() in existing_data['Email'].values:
